...
| product idea -> GitHub repo -> autonomous PR loop           |
+------------------------------------------------------------+
</pre>

# GitHub Loop Runner Skill

_A portable agent skill for bootstrapping GitHub-only autonomous development loops._
...
- **Runs milestone PR loops** - selects the first TODO from `docs/progress.md`, creates one branch, opens one PR, waits for CI, merges only when safe, updates progress, then re-reads state.
- **Uses Feedback Taxonomy** - classifies CI, review, mergeability, scope, progress, trace, hypothesis, repair, review, and stopper observations into allowed and forbidden next actions.
- **Adds Loop Trace** - records auditable evidence for milestone selection, context reads, PRs, CI attempts, feedback IDs, merge attempts, review decisions, and stopper outcomes.
...
Bootstrap runner docs, plan, progress, CI, and harness protocol
        |
        v
Generate a copy-paste runner prompt
        |
        v
Future agent reads repo docs and selects first TODO milestone
        |
        v
...
| Prompt integrity | Runner prompt contains GitHub-only, CI, feedback, trace, repair, hypothesis, and stopper rules. | `python scripts/validate_skill.py` |
...
| Agent harness research patterns | Trace every loop, classify root cause by harness layer, repair the harness separately, and gate process changes through hypotheses. |
...
| [Review and Renewal Loop](skills/github-loop-runner/references/review-and-renewal-loop.md) | [Loop hypotheses](skills/github-loop-runner/references/loop-hypotheses.md) |
...
| | Scope | GitHub-only | Progress loop | Feedback taxonomy | Trace / repair | Plan renewal |
| --- | --- | :---: | :---: | :---: | :---: | :---: |
| **GitHub Loop Runner Skill** | Repo bootstrap plus autonomous PR loop | Yes | Yes | Yes | Yes | Yes |
| Generic project prompt | One prompt | Sometimes | No | No | No | No |
| CI workflow template | Verification only | Yes | No | Partial | No | No |
| Agent memory file | Instructions only | Maybe | Manual | Manual | Manual | Manual |
